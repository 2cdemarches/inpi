...
export async function generatePdf(sourcePath, docType, client) {
  // 1. Convertir DOCX → HTML (mammoth fusionne les runs automatiquement)
  const result = await mammoth.convertToHtml({ path: sourcePath });
  let html = result.value;

  // 1b. Normaliser les fragments d'adresse découpés entre plusieurs <p> par mammoth
  // Ex: "135 Avenue de Flandre </p><p>75019 Paris" → "135 Avenue de Flandre 75019 Paris"
  ...
  // Supprimer les ancres HTML parasites (<a id="..."></a>) qui cassent les remplacements
  html = html.replace(/<a\s+id="[^"]*"><\/a>/g, '');

  // 2. Appliquer les remplacements dans le texte propre
  ...
  // 3. Envelopper dans un HTML complet avec CSS
...
body { font-family: Tahoma, Arial, sans-serif; font-size: 11pt; line-height: 1.6; color: #000; background: #fff; padding: 2.5cm 3cm; }
h1 { font-size: 13pt; font-weight: bold; text-align: center; margin: 1em 0 0.5em; }
h2 { font-size: 12pt; font-weight: bold; margin: 0.8em 0 0.3em; }
