...
async function buildHtml(sourcePath, docType, client, settings = {}) {
  ...
  // Statuts : ajouter tiret avant le bloc identité (Monsieur...) après "Le soussigné :"
  html = html.replace(
    /(<p[^>]*>Le soussigné\s*:<\/p>)(\s*<ul>)/,
    '$1<p class="tiret-identite">-</p>$2'
  );
  // Statuts : parenthèses autour du CP de naissance dans le bloc identité
  html = html.replace(
    /\{ville_naissance\}\s*\{cp_naissance\}/g,
    '{ville_naissance} ({cp_naissance})'
  );
