...
async function buildHtml(sourcePath, docType, client, settings = {}) {
  ...
  // Placeholders: ajouter parenthèses autour de {cp_naissance} dans le bloc identité (avant remplacement)
  html = html.replace('{ville_naissance} {cp_naissance}', '{ville_naissance} ({cp_naissance})');
  // Tiret avant le bloc identité (après "Le soussigné :")
  html = html.replace('soussigné :</p><ul>', 'soussigné :</p><p class="tiret-identite">-</p><ul>');
