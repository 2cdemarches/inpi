...
async function buildHtml(sourcePath, docType, client, settings = {}) {
  ...
  // Souscripteurs : remplacements directs des montants hardcodés
  if (docType === 'souscripteurs' && client.capital) {
    const cap = String(client.capital);
    // Cellules nb_actions standalone (sans "euros")
    html = html.split('<td><p>100</p></td>').join(`<td><p>${cap}</p></td>`);
    // Corps du texte
    html = html.split('souscription de 100 actions').join(`souscription de ${cap} actions`);
    html = html.split('la somme de 100 euros').join(`la somme de ${cap} euros`);
    html = html.split('la somme de100 euros').join(`la somme de ${cap} euros`);
    html = html.split('100 euros').join(`${cap} euros`);
    html = html.split('100 Euros').join(`${cap} Euros`);
  }
