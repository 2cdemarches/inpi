...
function getReplacements(docType, client, settings = {}) {
  // nb_actions doit toujours égaler capital (1 action = 1€) — corriger les anciennes données
  client = { ...client, nb_actions: client.capital || client.nb_actions };
  ...
  const common = [
    ...
    ['{date_premier_exercice}',   (client.date_premier_exercice || '31 décembre ' + (new Date().getFullYear())).replace(/decembre/gi, 'décembre').replace(/fevrier/gi, 'février').replace(/aout/gi, 'août')],
    ...
    ['{objet_social}',            objetSocialToHtml(client.objet_social)],
  ...
  const statuts = [
    ...common,
    // Premier exercice (Article 16)
    ['trente et un décembre 2025', client.date_premier_exercice || ('31 décembre ' + (new Date().getFullYear()))],
    ['trente et un décembre 2024', client.date_premier_exercice || ('31 décembre ' + (new Date().getFullYear()))],
    // Capital en lettres — pour les templates avec "cent" hardcodé (article 7)
    // Ces règles s'appliquent AVANT {capital_lettres} pour éviter les double-remplacements
    [`cent (${client.capital})`,      `${enLettres(client.capital)} (${client.capital})`],
    [`cent (100)`,                    `${enLettres(client.capital)} (${client.capital})`],
    [`de cent euros`,                 `de ${enLettres(client.capital)} euros`],
    [`cent euros`,                    `${enLettres(client.capital)} euros`],
    // {capital_lettres} EN DERNIER : après les règles "cent euros", pour ne pas être re-traité
    ['{capital_lettres}',             enLettres(client.capital)],
...
async function buildHtml(sourcePath, docType, client, settings = {}) {
  ...
  // Tiret avant le bloc identité (après "Le soussigné :")
  {
    const parts = html.split('soussign');
    if (parts.length > 1) {
      parts[1] = parts[1].replace(':</p><ul>', ':</p><p class="tiret-identite">-</p><ul>');
      html = parts.join('soussign');
    }
  }
  ...
  // Corriger "demeurant auX" (chiffre collé) → "demeurant au X"
  html = html.replace(/demeurant au(\d)/gi, 'demeurant au $1');
  ...
  // Souscripteurs : remplacements des valeurs hardcodées (capital = nb_actions car 1 action = 1€)
  if (docType === 'souscripteurs' && client.capital) {
    const cap = String(client.capital);
    // Cellules nb_actions (100 seul, sans "euros") — regex robuste tolérant espaces/attributs
    html = html.replace(/<td[^>]*>\s*<p[^>]*>\s*100\s*<\/p>\s*<\/td>/gi,
      `<td><p>${cap}</p></td>`);
    // Corps du texte
    html = html.replace(/souscription de \d+ actions/gi, `souscription de ${cap} actions`);
    html = html.replace(/la somme de\s*\d+\s*euros/gi, `la somme de ${cap} euros`);
    html = html.replace(/la somme de\s*\d+\s*Euros/gi, `la somme de ${cap} Euros`);
    // Montants dans cellules (déjà couverts par common mais au cas où)
    html = html.replace(/(<td[^>]*>\s*<p[^>]*>)\s*\d+\s*(euros|Euros)\s*(<\/p>\s*<\/td>)/gi,
      (_, open, unit, close) => `${open}${cap} ${unit}${close}`);
  }
...
p.entete2.page-break { page-break-before: always; margin-top: 0; font-size: 12pt; }
p.entete2.page-break ~ p.entete2,
p.entete2.page-break ~ p.entete3 { font-size: 12pt; }
...
p.efl-titre { font-weight: bold; text-align: center; font-size: 12pt; margin: 4pt 0 2pt; }
p.efl-cellule { margin: 0; font-size: 12pt; }
...
p.tiret-identite { margin: 4pt 0 2pt 0; }
p.objet-item { margin: 2pt 0; }
...
table { width: 100%; border-collapse: collapse; margin: 6pt 0; font-size: 12pt; }
...
h2 { font-size: 12pt; font-weight: bold; margin: 6pt 0 2pt; }
...
// ── Objet social → liste HTML ─────────────────────────────────────────────────
function objetSocialToHtml(text) {
  if (!text) return '';
  const items = text.split('\n').map(s => s.trim()).filter(Boolean);
  if (items.length <= 1) return text;
  return '</p>' + items.map(i => `<p class="objet-item">&nbsp;&nbsp;&nbsp;${i}</p>`).join('') + '<p>';
}
...
function enLettres(n) {
  ...
  let r = '';
  if (n >= 1000) { const m2 = Math.floor(n/1000); r += (m2===1?'mille':enLettres(m2)+' mille')+' '; n%=1000; }
...
// Garde l'ancienne signature pour compatibilité (retourne lettres + " euros")
function nombreEnLettres(n) { return enLettres(n) + ' euros'; }
