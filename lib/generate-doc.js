...
export async function generatePdf(sourcePath, docType, client) {
  ...
  // Centrer "STATUTS" (style Normal mais doit être centré comme en-tête)
  html = html.replace(/<p>(<strong>STATUTS<\/strong>)<\/p>/gi, '<p class="statuts-titre">$1</p>');
  // Ajouter classe .titre-section aux paragraphes TITRE I/II/...
  html = html.replace(/<p>(?=<strong>TITRE\s)/g, '<p class="titre-section">');

  // 4. HTML complet — @page gère les marges, body sans padding supplémentaire
...
/* ── En-têtes de la page de garde (styles Word En-teteSteActe2/3) ── */
p.entete2 {
  font-weight: bold;
  text-align: center;
  font-size: 10.5pt;
...
p.statuts-titre { text-align: center; font-weight: bold; font-size: 12pt; margin: 0 0 4pt 0; }
...
/* Sous-ligne du titre (après <br>) — letter-spacing comme dans Word */
